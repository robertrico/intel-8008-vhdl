...
; the monitor USART (IN 1 / OUT 9, 115200). Divide-by-zero traps return
; to the monitor. Self-modifying dispatch (RESULT patching) needs RAM.
;
;   make assemble-sample PROG=calc_ram
;   ./send_hex.py test_programs/samples/calc_ram.hex   (minicom closed)
;   then in minicom:  G 2100
;
; Multiply is "X". Type a space or = if it will not take input. 
;=============================================================================================== 
...
OPERA4:    mov c,a               ;SAVE "OPERATOR" CHARACTER IN REG "C" 
           ...
           mvi h,041             ;XXX = PAGE THIS PROGRAM LOCATED ON (0x21) 
           ...
           mvi h,040             ;STRUCTION LABELED "RESULT." THEN RESTORE 
...
DINPUT:    mvi h,040             ;** Set H to page of floating point working registers 
...
SUBTRA:    mov a,m               ;Fetch byte from register group A 
           ...
           ORG 2740H 
DERMSG:    jmp 0                 ;Attempted divide by zero - back to the monitor 
 
           ORG 2748H 
USERDF:    jmp 0                 ;Direct program flow after above error 
...
INPUT:      in 1                 ; USART RX: bit 7 = ready flag
            mov e,a              ; save flag + data
            ani 80h              ; ready?
            jz INPUT             ; no, keep polling
            mov a,e              ; ready byte is already MSB|char -
            ori 80h              ;   exactly the MSB-set ASCII expected
            ret         
...
;------------------------------------------------------------------------ 
ECHO:       ani 7fh              ; clear the most significant bit
            out 09h              ; USART sends immediately
            mov e,a              ; keep the character
            mvi a,14h            ; ~40 instructions > one 115200 frame
echodly:    sui 1
            jnz echodly
            mov a,e              ; return the 7-bit character in A
            ret 
