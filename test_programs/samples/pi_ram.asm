...
;;
;; RAM build for the b8008 monitor: everything relocated +0x2000 (the
;; original page-0 variables were WRITABLE and sat in what is now monitor
;; ROM), cout retargeted to the monitor USART (OUT 9), rst 7 reaches crout
;; through the monitor's RAM vector slot (installed at startup), and the
;; final hlt is a jmp 0 back to the monitor.
;;
;;    make assemble-sample PROG=pi_ram
;;    ./send_hex.py test_programs/samples/pi_ram.hex   (minicom closed)
...
; Original 1000 digit version (~25 min at 500 KHz):
;dec_len =   1000        ; 1000 decimal digits
;bin_len =   418         ; ceil(1001 / log(256)) + 1 (+ 1 if odd) = 418

; Short 50 digit version (~75 sec at 500 KHz):
dec_len =   50          ; 50 decimal digits
bin_len =   22          ; ceil(51 / log(256)) + 1 = 22, 22 % 4 = 2 ✓
...
;;   overlap with rst calls.
    ...
    org 2000h       ; variables page (was the rst-vector area at 0x00)
...
;; START of execution
    ...
    org 2040h       ; SCELBI entry - monitor: G 2040
main:
    lhi 3Fh         ; install "jmp crout" in the monitor RST 7 slot (0x3FF8)
    lli 0F8h        ;   so the rst 7 in printcr/prtbcd reaches crout
    lmi 44h         ; JMP opcode
    inl
    lmi lo(crout)
    inl
    ...
    printcr 2       ; 2 CRs
    jmp 0           ; all done - back to the monitor
...
OUTPORT     equ 09H                 ; monitor USART TX port (OUT 9, 115200)

;------------------------------------------------------------------------
; character output subroutine (monitor USART)
; the pacing loop covers one 115200-baud frame before the next OUT.
; uses A ONLY - B is preserved. prbcd keeps its BCD byte in B across the
; first cout call ("lba ... cal + ... lab"), so a cout that burns B (like
; the original bitbang's delay loop!) zeroes every second digit.
;------------------------------------------------------------------------
cout:       ani 7FH                 ; mask off the most significant bit
            out OUTPORT             ; USART sends immediately
            mvi a,14H               ; ~40 instructions > one character frame
cout_dly:   sui 1
            jnz cout_dly
            ret
