...
;
; RAM build for the b8008 monitor: relocated +0x2000, serial I/O through
; the monitor USART (IN 1 / OUT 9, 115200), page literals bumped to the
; 0x23-0x25 pages. The move lists self-modify as the machine learns -
; which is why it must run from RAM.
;
;   make assemble-sample PROG=hexspawn_ram
;   ./send_hex.py test_programs/samples/hexspawn_ram.hex   (minicom closed)
...
START:  mvi l,076                ;set pointer to intro message 
        ...
        mvi h,044                ;set pointer to move index page 
...
OHLF:   inr l                    ;advance pointer to 'O' board 
        ...
        mvi h,044                ;set pointer to move index table 
...
MOVE:   mov e,l                  ;save move location 
        ...
        mvi h,044                ;set pointer to active move list 
        mvi m,000                ;cancel last move 
        mvi h,045                ;set pointer to message "I WIN 
...
MSG1:   DB CR,LF,"HERE\'S THE BOARD" 
        ...
        ORG 2600H
;-----------------------------------------------------------------------------------------
; wait for a character from the monitor USART, echo it, and return it in A
; with the MSB set (the game compares against MSB-set ASCII, e.g. cpi 261).
; uses A and B.
;-----------------------------------------------------------------------------------------
INPUT:  in 1                     ; USART RX: bit 7 = ready flag
        mov b,a                  ; save flag + data
        ani 80h                  ; ready?
        jz INPUT                 ; no, keep polling
        mov a,b
        ani 7fh                  ; 7-bit character
        call PRINT               ; echo it
        ori 80h                  ; game expects MSB-set ASCII
        ret

;------------------------------------------------------------------------
; sends the character in A through the monitor USART (OUT 9).
; the pacing loop covers one 115200-baud frame before the next OUT.
; uses A and B; returns the 7-bit character in A.
;------------------------------------------------------------------------
PRINT:  ani 7fh                  ; clear the most significant bit
        out 09h                  ; USART sends immediately
        mov b,a                  ; keep the character
        mvi a,14h                ; ~40 instructions > one character frame
prtdly: sui 1
        jnz prtdly
        mov a,b                  ; return the 7-bit character in A
        ret
