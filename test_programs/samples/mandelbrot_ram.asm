...
;;; 
;:: RAM build: loaded by the b8008 monitor's L command, serial I/O via the 
;;; monitor USART (OUT 9, 115200 bps N-8-1). 
;;; 
;;; Jump to 2040H to start (monitor: G 2040). 
...
MANDELBROT: 
           mvi h,040             ; only mvi h in this pgm; all data on page 040 (0x20) - RAM build 
...
MANDONE:   call ECHO 
           ...
           jmp 0                 ; picture done - back to the monitor 
                                 ;LAA 
           mvi l,(STEPX+3)&0FFH 
           mov b,m 
           dcr b 
...
OUTPORT     equ 09H              ; monitor USART TX port (OUT 9, 115200 baud) 
;------------------------------------------------------------------------ 
; character output subroutine 
; sends the character in A out through the monitor USART. 
