;===========================================================================
; SCELBAL - SCELBI BASIC (Wadsworth, 1976) - b8008 RAM-resident port
;
; Base: Jim Loos's AS-assembler port, github.com/jim11662418/8008-SBC
; (scelbal-in-eprom.asm). b8008 transformations:
;   - variable pages relocated: OLDPG1/26/27/57 -> 0x1000-0x13FF
;   - user BASIC program buffer: 0x1400-0x1FFF (3KB)
;   - bitbang 2400bps serial replaced with monitor USART shims
;     (IN 1 poll / OUT 9 + one-frame pacing); CINP echoes and returns
;     MSB|char, CPRINT preserves A with MSB restored - the exact
;     contract documented in the SCELBAL manual (A and B only,
;     max two nesting levels)
;   - init-image pages re-ORG'd 0x3C00-0x3EFF (monitor scratch at
;     0x3F00 stays untouched)
;
;   Emulator oracle (2026-07-03): full session verified pre-silicon -
;   SCR / 10 PRINT "HELLO WORLD" / 20 GOTO 10 / LIST / RUN -> looping;
;   PRINT 2+2 -> 4.0; PRINT 355/113 -> 3.141593; FOR I=1 TO 5:
;   PRINT I*I -> 1 4 9 16 25. Zero port iterations required.
;
...
;------------------------------------------------------------------------            
            ...
            org 1800H               ; b8008_basic: SCELBAL ROM base (cold entry)
...
OUTPORT     equ 09H                 ; monitor USART TX: sends immediately

;-----------------------------------------------------------------------------------------
; b8008 character input for SCELBAL: wait for a character from the
; monitor USART, echo it, return it in A with the MSB set.
; uses A and B only; no nested calls (contract: A+B, max 2 levels).
;-----------------------------------------------------------------------------------------
CINP:       in INPORT               ; bit7 = ready flag
            rlc                     ; ready bit into carry
            jnc CINP                ; not ready - keep polling
            rrc                     ; undo the rotate, byte intact
            ori 80H                 ; SCELBAL wants the MSB set
            mov b,a                 ; keep the char (B is ours by contract)
            ani 7FH
            out OUTPORT             ; echo
            mov a,a                 ; pacing: one 115200 frame is ~87us,
            mov a,a                 ;   one instruction ~45us
            mov a,a
            mov a,a
            mov a,a
            mov a,a
...
;-----------------------------------------------------------------------------------------
; b8008 character output for SCELBAL: print A through the monitor
; USART; return with the original character (MSB set) in A - SCELBAL
; assumes CPRINT preserves the accumulator.
; uses A and B only; no nested calls.
;-----------------------------------------------------------------------------------------
CPRINT:     mov b,a                 ; save the character
            ani 7FH
            out OUTPORT
            mov a,a                 ; pacing (see CINP)
            mov a,a
            mov a,a
            mov a,a
            mov a,a
            mov a,a
            mov a,b                 ; original character back in A
            ori 80H                 ; MSB restored, per SCELBAL convention
            ret
...
OLDPG1      EQU 0C00H             ; b8008_basic: vars 0x0C00-0x0FFF
OLDPG26     EQU 0D00H             ; b8008_basic
OLDPG27     EQU 0E00H             ; b8008_basic
OLDPG57     EQU 0F00H             ; b8008_basic

BGNPGRAM    EQU 01H               ; b8008_basic: user programs 0x0100-0x0BFF
ENDPGRAM    EQU 0CH               ; b8008_basic: ~2.75KB of program space
...
NOLIST:    LLI 342                ;Load L with address of RUN in look up table
           ...
           CAL STRCP              ;Call string compare subroutine to see if first word in
           JFZ MONCHK             ;Input buffer is SCR. If not, b8008 checks MON next.
...
MONCHK:    LDI OLDPG26/400        ;b8008: reset pointers to the line input buffer
           LEI 000
           LLI 266                ;MON text at OLDPG1 offset 0xB6 (octal 266)
           LHI OLDPG1/400
           CAL STRCP              ;Compare first word against "MON"
           JFZ NOSCR              ;Not MON either - continue to SAVE/LOAD checks
           JMP 1000H              ;b8008: MON - jump to the machine monitor
...
page1:      DB 000,000,000,000
            ...
            db 1Dh dup 0                ; (was 21h dup 0)
            DB 3                        ; b8008: MON - drop to monitor
            DB 'M'+200
            DB 'O'+200
            DB 'N'+200
            DB 004                      ; CC FOR SAVE
...
;this page gets copied from EPROM to RAM at 0200H as OLDPG27 - jsl           
           ORG 3C00H          ; b8008: clear of monitor scratch at 0x3F00
page27:    DB 3
           DB 'R'+200
           DB 'E'+200
           DB 'M'+200
