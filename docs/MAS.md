...
- **advance_state vs cycle_done:** `advance_state` = instruction complete; the ONLY gate under which an interrupt may enter (T3/T4/T5 → T1I). `cycle_done` = cycle over mid-instruction; deliberately no interrupt check. Both latch in machine_cycle_control, clear at every T1/T1I entry. `(PSL: state_timing P4 priority chains; machine_cycle latch rules)` Mutual exclusion of the two is **composition-level only** — module-locally false (HLT-at-T3 path). `(PSL: formal/mcc_stg_cluster — proven bmc-120 on the IR+decoder+MCC+STG cluster under the ir_load discipline; building the proof surfaced that the mutex DEPENDS on (a) single IR load per fetch at phi1_falling, (b) T1I always jamming, (c) no reload at a jam or data cycle's T3, and (d) one phi pulse per half — each discharged by the cocotb suites.)`
